 ...
 * are included.
 */
TEMPLATE_TEST_CASE("Find all elements not matching a criterion in a collection of characters using find_all_if_not",
                   "[find_all_if_not]",
                   (std::string),
                   (std::vector<char>),
                   (std::deque<char>),
    ...
    for (auto testcase : cases) {
        ...
        SECTION(testcase.case_title) {
            ...
            // ===== Run the find_all_if_not algorithm and check that the number of found elements are correct.
        ...
        SECTION(testcase.case_title + " (Parallel)") {
            ...
            // ===== Run the find_all_if_not algorithm and check that the number of found elements are correct.
            std::vector<decltype(container.begin())> results;
            trl::find_all_if_not(std::execution::seq, container.begin(), container.end(), std::back_inserter(results),
                                 [&](char c) {
                                     return c != testcase.search_item;
                                 });
            REQUIRE(results.size() == testcase.item_locations.size());
